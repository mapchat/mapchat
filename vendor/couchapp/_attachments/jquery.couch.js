      ...
      return /** @lends $.couch.db */{
        ...
        saveDoc: function(doc, options) {
          ...
          var beforeSend = fullCommit(options),
              update = options.update && options.update.split('/', 2),
              update_url = update ? '_design/' + update[0] + 
                                    '/_update/' + update[1] + '/' : '';
          ...
          if (doc._id === undefined) {
            var method = "POST";
            var uri = this.uri + update_url;
          } else {
            var method = "PUT";
            var uri = this.uri + update_url + encodeDocId(doc._id);
          }
